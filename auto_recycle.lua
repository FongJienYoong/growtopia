...
-- Function to attempt trashing an item
function trashItem(item)
    LogToConsole("Attempting to trash item: " .. item.id .. " with amount: " .. item.amount)
    
    -- Send packet to trash the item (not drop it visually)
    SendPacket(2, "action|drop\n|itemID|" .. item.id)  -- Using drop packet, but see if it gets removed instantly
    
    -- Simulate no visual drop, making it disappear directly
